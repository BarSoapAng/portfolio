...
- Run `pnpm run lint` when `*.js`, `*.jsx`, `*.ts`, or `*.tsx` files change. Treat it as the default validation step for `src/` code edits.
- Run `pnpm run build` when route entrypoints, navigation behavior, global layout or CSS, rendering behavior, metadata, or production asset loading changes.
