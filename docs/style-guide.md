...
- When adjacent sections or icon-button groups need a stronger boundary, separate them
  with a single divider line. Do not add a background or enclosing border around each
  icon-button group.
- Prefer transparent, borderless icon buttons. Use spacing, icon color, and visible
  focus or selected states to communicate interaction. This does not apply to text
  buttons, which may use fills and borders to communicate their hierarchy.
